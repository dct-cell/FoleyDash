...
from src.ext.rotary_embeddings import compute_rope_rotations
from src.model.embeddings import TimestepEmbedder
from src.model.low_level import MLP, ChannelLastConv1d, ConvMLP
from src.model.transformer_layers import FinalBlock, JointBlock, MMDitSingleBlock
...
class FoleyDash(nn.Module):
    def __init__(
        ...
        *,
        ...
        if latent_mean is None:
            ...
            latent_mean = torch.full((1, 1, latent_dim), torch.nan)
            latent_std = torch.full((1, 1, latent_dim), torch.nan)
        else:
            assert latent_std is not None
            assert latent_mean.numel() == latent_dim, (
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.s_embed.mlp[0].weight, std=0.02)
        nn.init.normal_(self.s_embed.mlp[2].weight, std=0.02)
    ...
    def predict_flow(
        ...
        t_embed = self.t_embed(t).unsqueeze(1)
        s_embed = self.s_embed(s - t).unsqueeze(1)

        global_c = t_embed + s_embed + global_c.unsqueeze(1)  # (B, D)
    ...
    def forward(
        ...
        flow = self.predict_flow(latent, t, s, conditions)
        return flow
    ...
    def get_empty_conditions(
        self, bs: int, *, negative_text_features: torch.Tensor = None
    ) -> PreprocessedConditions:
    ...
    def ode_wrapper(
        ...
        t = t * latent.new_ones((latent.size(0), 1, 1))
        s = s * latent.new_ones((latent.size(0), 1, 1))

        if cfg <= 1.0:
            return self.predict_flow(latent, t, s, conditions)
        else:
            return cfg * self.predict_flow(latent, t, s, conditions) + (
                1 - cfg
            ) * self.predict_flow(latent, t, s, self.get_empty_conditions(latent.size(0)))
...
def get_model(variant: str, sample_rate: str, **kwargs) -> FoleyDash:
    clip_dim = 1024
    sync_dim = 768
    text_dim = 1024
