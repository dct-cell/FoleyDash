...
from util.av_tool import ImageInfo, VideoInfo, read_frames, reencode_with_audio
from util.download import download_model_if_needed

from .model.flow_matching import FlowMatching
from .model.networks import FoleyDash
...
def generate(
    ...
    *,
    ...
    preprocessed_conditions = net.preprocess_conditions(
        clip_features, sync_features, text_features
    )

    cfg_ode_wrapper = lambda t, s, x: net.ode_wrapper(  # noqa: E731
        t,
        s,
        x,
        preprocessed_conditions,
    )
    x0 = fm.to_data(cfg_ode_wrapper, x1)
    x0 = net.unnormalize(x0)
    spec = feature_utils.decode(x0)
